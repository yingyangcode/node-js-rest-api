        ...
        _data.read('users', phone, function(err, userData){
            if(!err && userData) {
                ...
                if(hashedPassword == userData.hashedPassword) {
                    // If valid create a new token with a random name. Set expiration date to be 1 hour in the future
                ...
                _data.read('users', userPhone, function(err, userData){
                    if(!err && userData) {
                        ...
                        if(userChecks.length < config.maxChecks) {
                            ...
                            // Store each check with a ref to its creator and make sure the creators also have a ref on their objects to each of their checks
                            // This is a noSql way of storing things in a key-value store
                            const checkObject = {
                                'id' : checkId,
                                'userPhone' : userPhone,
                                'protocol' : protocol,
